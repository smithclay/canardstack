...
start_server() {
  ...
  CANARDSTACK_BIND="127.0.0.1:${PORT}" \
    ...
    CANARDSTACK_DUCKDB_PATH="${data_dir}/canardstack.duckdb" \
    CANARDSTACK_STORAGE_DIR="${data_dir}/storage" \
    CANARDSTACK_RAW_SPOOL_DIR="${data_dir}/raw-spool" \
    CANARDSTACK_MAX_FLUSH_AGE_MS=500 \
    CANARDSTACK_HIGH_PRESSURE_FLUSH_AGE_MS=200 \
    CANARDSTACK_IMMUTABLE_SEGMENT_MAX_AGE_MS=500 \
    CANARDSTACK_SCHEDULER_WATCHDOG_MS=100 \
