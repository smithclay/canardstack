...
gcloud infra-manager deployments apply projects/PROJECT_ID/locations/us-central1/deployments/canardstack \
  ...
  --git-source-ref=main \
  --input-values=project_id=PROJECT_ID,region=us-central1,bucket_name=my-canardstack-ducklake,image=ghcr.io/<owner>/canardstack:<version>,catalog_image=ghcr.io/<owner>/duckdb-quack:<version>,api_key=REPLACE_ME,admin_api_key=REPLACE_ME_TOO,quack_token=REPLACE_ME_THREE
```
