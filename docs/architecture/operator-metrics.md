...
| `canardstack_ingest_stage_total` | Counter | `request_kind`, `stage` | Per-request ingest lifecycle funnel: one increment as each request ENTERS a stage, where `stage` is an `IngestStage` value (shared with the `stage` field on `tracing` events). The per-request happy-path stages are a monotone funnel, so `admitted_not_spooled` >= `durably_spooled` >= `worker_dispatched`+`inline_processed` >= `transformed` >= `arrow_buffered`; gaps between adjacent stages count requests that were rejected or left pending at that hop. `terminally_rejected_checkpointed` counts requests disposed of for a terminal payload fault. |
| `canardstack_ingest_seal_stage_total` | Counter | `stage` | Per-seal-operation lifecycle funnel (NOT per record — do not confuse with `canardstack_raw_spool_checkpointed_records_total`, which counts records): one increment per seal operation that reaches a milestone, where `stage` is a seal-phase `IngestStage` value. Counts only seals that captured at least one record (idle scheduler seals are excluded), so `captured_for_seal` >= `ducklake_committed` >= `raw_spool_checkpointed`. `committed_not_checkpointed > 0` is the duplicate-risk signal: the rows committed to DuckLake but the raw-spool checkpoint failed, so those records replay as duplicate rows on a future restart (v0 does not dedup). |
