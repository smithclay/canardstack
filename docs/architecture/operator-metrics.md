...
| `canardstack_ingest_stage_total` | Counter | `request_kind`, `stage` | Per-request durable-boundary funnel: one increment as each request crosses a real durability boundary. The happy-path boundaries are a monotone funnel, so `accepted` >= `spooled` >= `transformed` >= `buffered`; gaps between adjacent boundaries count requests that were rejected or left pending there. Terminally-rejected requests are dropped from this funnel (covered by `canardstack_raw_spool_checkpointed_records_total{reason}` and `canardstack_ingest_requests_total{reason}`). |
| `canardstack_ingest_seal_stage_total` | Counter | `stage` | Per-seal-operation durable-boundary funnel (NOT per record — do not confuse with `canardstack_raw_spool_checkpointed_records_total`, which counts records): one increment per seal operation that crosses a boundary. Counts only seals that captured at least one record (idle scheduler seals are excluded), so `committed` >= `checkpointed`. `duplicate_risk > 0` is the at-least-once hazard marker: the rows committed to DuckLake but the raw-spool checkpoint failed, so those records replay as duplicate rows on a future restart (v0 does not dedup). |
